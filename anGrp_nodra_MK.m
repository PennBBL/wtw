function [] = anGrp_nodra_MK()
% analyze results of a wtw experiment

cd /data/jux/BBL/studies/reward/processedNeuroec/wtw
% set path to subfunctions
addpath('/data/jux/BBL/studies/reward/processedNeuroec/wtw/scripts');
addpath('/data/jux/BBL/studies/reward/processedNeuroec/wtw/scripts/subFx_analysis');
addpath('/data/jux/BBL/studies/reward/processedNeuroec/wtw/scripts/subFx_load');
...
% examine the assignment of subjects to projects
...
for g = 1:nGrps
    ...
    for s = 1:gN
        ...
        % Mack added: date for each subj and compiled dates
        date = grpData(sIdx).sessionTime 
        alldates.(gName){s,1} = date;  
        ...
        for b = 1:nBks
            ...
            % Mack added to format by quit/win choice
        ...
        if runTest.RunningWTW
            ...
            % Mack added: separate structure for win/quit
            tmp1=ismember({sData.trialResult},'win');
            tmp2=ismember({sData.trialResult},'quit');
            for i=1:length(sData)
                sData(i).outcomeWin=tmp1(i);
                sData(i).outcomeQuit=tmp2(i);
            end
...
!Rscript /data/jux/BBL/studies/reward/processedNeuroec/wtw/scripts/txtOutputProcesser_nodra.R
