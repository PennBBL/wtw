...
elseif kmX(end)<truncPt && kmF(end)>0
    ...
    %%% changed this on 2017-05-22
    %%% the 4 lines below were the old way of doing it
    %%% caused an artifactual drop on the very last point of the curve
%     lastPt = find(kmX>truncPt,1,'first'); 
%     kmX((lastPt+1):end) = [];
%     kmF((lastPt+1):end) = [];
%     kmX(end) = truncPt;
    %%% revised:
    lastPt = find(kmX<=truncPt,1,'last'); 
    kmX((lastPt+1):end) = [];
    kmF((lastPt+1):end) = [];
    kmX(end+1,1) = truncPt;
    kmF(end+1,1) = kmF(end);
